...
import {
  Plus, Trash2, Loader2, Library, ChevronDown, ChevronUp, Upload, Download, ClipboardPaste,
} from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {showBank ? <ChevronUp size={20} className="shrink-0 text-muted-foreground/50" /> : <ChevronDown size={20} className="shrink-0 text-muted-foreground/50" />}
      </button>

      {showBank && questions.length === 0 && (
        <div className="rounded-2xl border border-dashed border-border bg-card/60 px-4 py-7 text-center text-sm text-muted-foreground">
          {ar ? "بنك الأسئلة فارغ — أضف أول سؤال من الخيارات أدناه." : "The question bank is empty — add your first question below."}
        </div>
      )}
