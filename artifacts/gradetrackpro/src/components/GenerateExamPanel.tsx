...
interface Props {
  ...
  onGeneratedForms?: (items: { exam: OmrExam; form: GeneratedForm }[]) => void;
  onCreateExam: (input: {
...
export default function GenerateExamPanel({ course, bankCourseIds, sheetHeader, componentOptions, onGeneratedForms, onCreateExam, onSetAnswerKey, buildExam }: Props) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGenerated(out);
      onGeneratedForms?.(out.flatMap(({ exam, form }) => exam ? [{ exam, form }] : []));
      toast.success(
        ar
  ...
  return (
    <>
      ...
      {open && (
        <div id="gtp-exam-generator" className="space-y-3 rounded-2xl border border-success/40 bg-success/5 p-4 shadow-sm">
          <input
            value={genTitle}
          ...
          <div className="flex gap-2">
              {([
                ...
                { key: "manual", label: ar ? "إضافة أسئلة يدويًا" : "Add questions manually" },
            ] as const).map((m) => (
              <button
                key={m.key}
          ...
          {genPickMode === "manual" && (
            <div className="space-y-2 rounded-xl border border-border bg-card p-3">
               <p className="text-xs font-bold text-foreground">
                 {ar ? "اختر أسئلة الاختبار من بنك الأسئلة:" : "Choose the exam questions from the question bank:"}
               </p>
