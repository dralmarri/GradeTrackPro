...
import {
  Plus, Printer, FileText, Trash2, KeyRound, ScanLine, Loader2, CheckCircle2, Pencil, History, BarChart3,
...
export default function OmrExamsPage({ course, bankCourseIds, onApplyScore, onLearnNumber }: Props) {
  ...
  const [questionPapers, setQuestionPapers] = useState<Record<string, GeneratedForm>>(() => {
    try {
      const saved = localStorage.getItem("gtp_question_papers");
      return saved ? JSON.parse(saved) as Record<string, GeneratedForm> : {};
    } catch {
      return {};
    }
  });

  const rememberQuestionPapers = (items: { exam: OmrExam; form: GeneratedForm }[]) => {
    if (!items.length) return;
    setQuestionPapers((current) => {
      const next = { ...current };
      items.forEach(({ exam, form }) => { next[exam.id] = form; });
      try { localStorage.setItem("gtp_question_papers", JSON.stringify(next)); } catch { /* storage is optional */ }
      return next;
    });
  };

  const openQuestionPaperGenerator = () => {
    setFormsOpen(true);
    setTimeout(() => document.getElementById("gtp-exam-generator")?.scrollIntoView({ behavior: "smooth", block: "center" }), 0);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="flex w-full items-center gap-4 rounded-[24px] border border-border bg-card p-3 text-start shadow-sm transition-colors hover:bg-muted/40 sm:gap-4 sm:p-3.5"
      >
      ...
      <button
        ...
        className="flex w-full items-center gap-4 rounded-[24px] border border-border bg-card p-3 text-start shadow-sm transition-colors hover:bg-muted/40 sm:gap-4 sm:p-3.5"
      >
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-success/10 text-success sm:h-12 sm:w-12">
          <Wand2 size={22} />
        </span>
        <span className="min-w-0 flex-1">
      ...
      <GenerateExamPanel
        ...
        onGeneratedForms={rememberQuestionPapers}
        ...
        <div className="space-y-3 rounded-2xl border border-border bg-card p-4 shadow-sm">
          ...
          <button
            ...
            onClick={() => {
              setShowCreate(false);
              setFormsOpen(true);
              setTimeout(() => document.getElementById("gtp-exam-generator-toggle")?.scrollIntoView({ behavior: "smooth", block: "center" }), 50);
            }}
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-success/40 bg-success/10 px-3 py-2.5 text-xs font-bold text-success transition-colors hover:bg-success/15"
          >
            <Wand2 size={15} />
            {ar ? "إضافة أسئلة من بنك الأسئلة" : "Add questions from the question bank"}
          </button>
          <p className="text-center text-[11px] text-muted-foreground">
            {ar ? "استخدم هذا الخيار لإنشاء ورقة أسئلة فعلية، وليس ورقة إجابة فارغة." : "Use this to create an actual question paper, not an empty answer sheet."}
          </p>
          ...
          <div key={exam.id} className="rounded-2xl border border-border bg-card p-4 shadow-sm">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <button
                  onClick={() => {
                    const paper = questionPapers[exam.id];
                    if (paper) {
                      if (!printQuestionPaper(exam.title, paper, sheetHeader(), exam.maxScore)) {
                        toast.error(ar ? "اسمح بالنوافذ المنبثقة للطباعة" : "Allow pop-ups to print");
                      }
                    } else {
                      toast.info(ar ? "ولّد ورقة الأسئلة من بنك الأسئلة أولاً" : "Generate the question paper from the question bank first");
                      openQuestionPaperGenerator();
                    }
                  }}
