...
export const BUBBLE_R = 4.5;      // bubble radius (mm) — readable on printed A4 sheets
...
export function gridSpec(exam: GridExam): GridSpec {
  ...
  // Short exams should use the available sheet height instead of leaving
  // every question crowded at the top. The scanner imports this same spec,
  // so the larger pitch remains deterministic for printed answer sheets.
  const pitch = rows > 1
    ? Math.max(Q_ROW_PITCH, Math.min(30, avail / (rows - 1)))
    : Q_ROW_PITCH;
