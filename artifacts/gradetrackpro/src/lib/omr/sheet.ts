...
function answerBubble(x: number, y: number, letter: string): string {
  return `<circle cx="${x}" cy="${y}" r="${BUBBLE_R}" fill="#fff" stroke="${INK}" stroke-width="0.6"/>` +
    svgText(x, y + 1.25, letter, 3.5, `fill="#555" text-anchor="middle" font-weight="600"`);
}
...
function brand(header?: SheetHeader): string {
  ...
  return titleBlock + logo +
    // Keep the Latin brand name LTR inside the RTL sheet so it does not
    // render backwards into the GTP logo.
...
function questionGrid(exam: OmrExam): string {
  ...
  for (let q = 0; q < exam.questionCount; q++) {
    ...
    out.push(`<circle cx="${nx}" cy="${p0.y}" r="3.6" fill="#fff" stroke="${INK}" stroke-width="0.55"/>`);
    out.push(svgText(nx, p0.y + 1.25, String(q + 1), 3.5, `text-anchor="middle" font-weight="700"`));
